...
-export([start/0, uid/0, getRecordByKey/2, getRecordsByMatch/1, 
         ...
         updateRecord/1, insertRecords/2, exists/2, timestamp/0, to_hex/1, execute/1]).
...
deleteAndInsertRecords(DList, IList, ITransform) ->
  {atomic,Status} = db_util:execute(
    fun() ->
      lists:foreach(fun(X) -> mnesia:delete_object(X) end, DList),
      lists:foreach(fun(X) -> mnesia:write(ITransform(X)) end, IList)
    end
  ),
  Status.  
